...
class VppPapiProvider(object):
            ...
            if install_dir:
                for root, dirnames, filenames in os.walk(install_dir):
                    for filename in fnmatch.filter(filenames, '*.api.json'):
                        vpp_jsonfiles.append(os.path.join(root, filename))

        levelname = logging.getLevelName(self.LOG.level)
        self.vpp = VPP(vpp_jsonfiles, logger=self.LOG, loglevel=levelname,
    ...
    def wait_for_event(self, timeout, name=None):
        ...
        if name:
            self.LOG.debug("Expecting event '%s' within %ss", name, timeout)
        else:
            self.LOG.debug("Expecting event within %ss", timeout)
        if self._events:
            self.LOG.debug("Not waiting, event already queued")
    ...
    def get_node_graph(self):
        ...
        """
        return self.api(
            self.papi.get_node_graph, {})

    def sw_interface_vhost_user_dump(self):
        """
        vhost-user dump
        :return:
        """
        return self.api(
            self.papi.sw_interface_vhost_user_dump, {})
    ...
    def create_vhostuser_socket(self, sock_file, server, mac_address, tag):
        """ Create vhostuser socket
        """
        return self.api(self.papi.create_vhost_user_if,
                        {'sock_filename': sock_file,
                         'is_server': server,
                         'use_custom_mac': 1,
                         'mac_address': mac_address,
                         'tag': tag})

    def delete_vhostuser_socket(self, if_idx):
        """ Delete vhostuser socket
        """
        return self.api(self.papi.delete_vhost_user_if,
                        {'sw_if_index': if_idx})

    def show_vhostuser(self):
        """ Show vhostuser socket
        """
        return self.api(self.papi.show_vhost_user_if,
                        {})
    ...
    def af_packet_create(self, lnx_veth_name, mac_address):
        """Create af_packet interface
        """
        return self.api(self.papi.af_packet_create,
                        {'host_if_name': lnx_veth_name,
                         'hw_addr': mac_address})

    def af_packet_delete(self, lnx_veth_name):
        """Delete af_packet interface
        """
        return self.api(self.papi.af_packet_delete,
                        {'host_if_name': lnx_veth_name})

    def set_interface_tag(self, sw_if_index, tag):
        """Set interface tag
        """
        return self.api(self.papi.sw_interface_tag_add_del,
                        {'sw_if_index': sw_if_index,
                         'is_add': 1,
                         'tag': tag})
