...
class StateWatcher(FileWatcher):
    ...
    def process(self, files):
        ...
        if not os.path.isfile(self.netsfile):
            LOG.debug("Waiting for instance network state file: %s",
                      self.netsfile)
            return
...
class AsMetadataManager(object):
    ...
    def clean_files(self):
        def rm_files(dirname, extension):
            ignorelist = [STATE_FILENAME_SVC, STATE_FILENAME_NETS]
            try:
